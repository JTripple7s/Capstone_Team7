...
class LineFollowingNode(Node):
    ...
    def enter_srv_callback(self, request, response):
        ...
        #self.pwm_controller([1, self.servo_data['servo1'] + 350], [2, self.servo_data['servo2']])
        with self.lock:
            self.stop = False            
            self.is_running = False
            self.color_picker = None
    ...
    def image_callback(self, ros_image):
        ...
        with self.lock:
            ...
            if target_detected:
                ...
                if self.is_running:
                    self.no_target_time += 1  # 增加计数器
                    if self.no_target_time >= self.no_target_threshold and self.exit_funcation:  # 假设30Hz的图像频率
                        self.exit_funcation = False
