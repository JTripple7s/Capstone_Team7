...
class QRCodeDetectNode(Node):
    def __init__(self, name):
        ...
        self.mecanum_pub = self.create_publisher(Twist, '/cmd_vel', 1, callback_group=callback_group)
        self.result_publisher = self.create_publisher(Image, '~/image_result', 1, callback_group=callback_group)
        self.enter_srv = self.create_service(Trigger, '~/enter', self.enter_srv_callback, callback_group=callback_group)
        self.exit_srv = self.create_service(Trigger, '~/exit', self.exit_srv_callback, callback_group=callback_group)
        self.start_recognition_srv = self.create_service(SetBool, '~/start_recognition', self.start_recognition_srv_callback, callback_group=callback_group)
        self.set_running_srv = self.create_service(SetBool, '~/set_running', self.set_running_srv_callback, callback_group=callback_group)
        ...
        self.create_service(Trigger, '~/init_finish', self.get_node_state, callback_group=callback_group)
    ...
    def enter_srv_callback(self, request, response):
        ...
        with self.lock:
            ...
            if self.image_sub is None:
                self.image_sub = self.create_subscription(Image, '/image_raw', self.image_callback, 1)  # 摄像头订阅
    ...
    def exit_srv_callback(self, request, response):
        ...
        # 1. 停止所有活动中的定时器
        # with self.lock:  # 移除定时器相关代码
        #     if hasattr(self, '_timers'):  # 检查 _timers 属性是否存在
            ...
            if self.is_running:
                ...
                for obj in decoded_objects:
                    ...
                    if len(points) == 4:
                        ...
                        hull = cv2.convexHull(np.array([point for point in obj.polygon], dtype=np.float32))
                        cv2.polylines(result_image, [np.int32(hull)], isClosed=True, color=(0, 255, 0), thickness=3)
                    cv2.putText(result_image, obj.data.decode("utf-8"), (obj.rect[0], obj.rect[1]), 
